...
import sardine
from sardine.wrappers import IdealState, TopK
...
def make_env(env_id, seed, full_observable = True):
    def thunk():
        env = gym.make(env_id)
...
"""
    ...
    # single_env = make_env(args.env_id, args.seed)()
    # Here, we assume that all dimensions of the cartesian product have the same actions !!
    n_actions = envs.single_action_space.shape[0]
    high = envs.single_action_space[0].n - 1

    # action_wrapper = TopK(single_env, min_action = np.zeros(n_actions), max_action = high *np.ones(n_actions))
    action_wrappers = [TopK(env, min_action = np.zeros(n_actions), max_action = high *np.ones(n_actions)) for env in envs.envs]
    ...
    actor = Actor(
        action_dim=10, #Yes, to do a dot product
        action_scale=jnp.array((1.0) / 2.0),
        action_bias=jnp.array((1.0) / 2.0),
    )
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            #Take the wrapper for the first env
            actions = action_wrappers[0].action((actor.apply(actor_state.params, obs)[0])).reshape(1,-1)
            print(actions)
            # actions = np.array(
            #     [
            #         (
        ...
        next_obs, rewards, terminations, truncations, infos = envs.step(list(actions))
